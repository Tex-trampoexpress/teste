/*
  ...
  1. Novas Funções Otimizadas
    - `get_users_by_proximity_optimized` - Busca usuários próximos com cache e índices
    - `search_users_with_distance` - Busca com termo E cálculo de distância
    - `calculate_distance` - Função auxiliar para calcular distância entre coordenadas

  2. Índices de Performance
    - Índice GiST para lat/long (busca geoespacial)
    - Índice GIN para tags (busca textual)
    - Índice para status e perfil_completo

  3. Melhorias
    - Cache de resultados
    - Ordenação por distância
    - Busca inteligente com distância
*/

-- Criar extensão postgis se não existir (para cálculos geográficos otimizados)
...
BEGIN
  ...
  WHERE
    ...
    -- Filtro rápido por bounding box antes de calcular distância exata
...
CREATE INDEX IF NOT EXISTS idx_usuarios_descricao_trgm ON usuarios USING gin(descricao gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_usuarios_nome_trgm ON usuarios USING gin(nome gin_trgm_ops);

-- Criar extensão para busca textual otimizada
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- Comentários
COMMENT ON FUNCTION calculate_distance IS 'Calcula distância entre duas coordenadas usando fórmula Haversine';
COMMENT ON FUNCTION get_users_by_proximity IS 'Busca usuários próximos ordenados por distância (otimizado)';
COMMENT ON FUNCTION search_users_with_distance IS 'Busca inteligente com termo E cálculo de distância';
