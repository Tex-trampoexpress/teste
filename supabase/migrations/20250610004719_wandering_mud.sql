/*
  ...
  1. Nova Estrutura
    ...
    - Campos adicionais: `perfil_completo`, `ultimo_acesso`, `verificado`
    - Índices otimizados para performance

  2. Funções Avançadas
    - `get_users_by_proximity()` - Busca por proximidade geográfica
  ...
  4. Dados de Teste
    - 3 perfis realistas incluídos
    - Diferentes localizações e tags
*/
...
-- Remover funções com CASCADE para resolver dependências
DROP FUNCTION IF EXISTS get_users_by_proximity(numeric, numeric, numeric) CASCADE;
DROP FUNCTION IF EXISTS search_usuarios(text, text[], text, integer) CASCADE;
DROP FUNCTION IF EXISTS update_atualizado_em() CASCADE;
DROP FUNCTION IF EXISTS update_ultimo_acesso() CASCADE;
DROP FUNCTION IF EXISTS check_perfil_completo() CASCADE;
