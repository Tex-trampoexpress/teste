/*
  # Banco de Dados Otimizado para TEX - Fluxo Atual

  1. Nova Estrutura da Tabela `usuarios`
    - `id` (uuid, primary key) - ID único do usuário
    - `nome` (text, required) - Nome completo do profissional
    - `whatsapp` (text, unique, required) - Número do WhatsApp (chave única)
    - `descricao` (text, optional) - Descrição dos serviços
    - `tags` (text[], default empty) - Tags de especialidades
    - `foto_url` (text, optional) - URL da foto de perfil
    - `localizacao` (text, optional) - Localização textual
    - `status` (text, default 'available') - Status: available/busy
    - `latitude` (numeric, optional) - Coordenada de latitude
    - `longitude` (numeric, optional) - Coordenada de longitude
    - `criado_em` (timestamptz, auto) - Data de criação
    - `atualizado_em` (timestamptz, auto) - Data de atualização
    - `ultimo_acesso` (timestamptz, auto) - Último acesso do usuário
    - `perfil_completo` (boolean, default false) - Se o perfil está completo
    - `verificado` (boolean, default false) - Se o usuário foi verificado

  2. Índices Otimizados
    - Índice único no WhatsApp para login rápido
    - Índice GIN nas tags para busca eficiente
    - Índice geoespacial para proximidade
    - Índice no status para filtros
    - Índice composto para buscas complexas

  3. Segurança RLS
    - Políticas para leitura pública (perfis disponíveis)
    - Políticas para edição própria (apenas seu perfil)
    - Políticas para criação autenticada
  ...
  5. Triggers
    - Auto-atualização do campo `atualizado_em`
    - Auto-atualização do campo `ultimo_acesso`
...
-- Função para busca por proximidade (otimizada)
